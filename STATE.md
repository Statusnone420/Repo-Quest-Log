...
## Current Focus
Major product correctness pass for generic repos and repo trust. RepoLog is read-only on open/scan/render/analyze, runtime/digest state moved to app-owned cache, generic repos now show onboarding/readiness context instead of empty HUD panels, and Tuneup generates a one-shot agent-ready docs repair prompt.
