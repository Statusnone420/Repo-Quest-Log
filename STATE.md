...
## Current Focus
Daily-use Workspace Signals rescue pass. RepoLog desktop now favors observable repo activity over fake agent liveness: broad metadata-only activity events feed `workspaceSignals` and `recentActivity`, desktop shows Workspace Signals, Recent Activity, Agent Docs, Prompt Palette, actionable empty-Now repair, and `Blocked: None` no longer renders as a blocker.
