...
> Session 2026-06-01 (Workspace Signals mode correction): Corrected the Full Signal System UX after visual review. The four agent work modes (`Building`, `Reviewing`, `Researching`, `Idle`) now live inside the `Workspace Signals` strip as automatic read-only status, not selectable user settings. The strip reads as "what is the agent doing right now?" first, then shows evidence: signal state, last edit, edits/min, files touched, outside-scope count, edit churn, and the 5m/15m/30m Activity timeline. Mode inference is local: fresh watcher activity means `Building`, dirty Git files without active edits means `Reviewing`, task context before edits means `Researching`, and no evidence means `Idle`. Removed the manual workspace-mode storage module, preload method, IPC handler, and click/toggle UI. Code review found stale spread-window activity could keep `Building`, fallback scanned docs could force `Reviewing`, and one watcher test could strand a handle on timeout; all were fixed with regression coverage. Verification passed: `npm run build`, `npm run lint`, `npm test` (118 tests / 24 files), and `npm run test:release-smoke` (20 tests / 3 files). Browser/app visual QA was not launched per human direction.

> Session 2026-06-01 (Full Signal System visual pass): Added meaning-bearing desktop signal visuals without launching the app. Workspace Signals now computes 5m/15m/30m timeline windows from local watcher events while preserving the existing 30m trend data. The desktop renderer now shows inferred workspace modes (`Building`, `Reviewing`, `Researching`, `Idle`), an interactive Activity timeline, a Scope map tile, Agent Docs health rails, readiness meters, and `Diff` actions that open a capped read-only diff drawer. Added `src/desktop/git-diff.ts` for safe `git diff --no-ext-diff --unified=3` previews; Electron preload/main now expose `repolog:get-file-diff`. Added a deterministic `.repolog.json` mtime guard so the activity watcher reloads config even when chokidar misses the root dotfile event under full-suite load. Code review found large-diff and untracked-file preview gaps; both were fixed with streaming/capped diff reads and synthetic added-file previews. Added coverage for timeline windows, path-safe diff previews, large diff truncation, untracked file previews, renderer surfaces, desktop IPC wiring, and watcher config reload. Verification passed: `npm run build`, `npm run lint`, `npm test` (117 tests / 24 files), `npm run test:release-smoke` (20 tests / 3 files), and `git diff --check` with CRLF warnings only.
